    ...
    public class UtScriptBuilder
    {
        ...
        public void TestEmitPushBool()
        {
            using (ScriptBuilder script = new())
            {
                ...
                CollectionAssert.AreEqual(new byte[] { (byte)OpCode.PUSH1, (byte)OpCode.NOT, (byte)OpCode.NOT }, script.ToArray());
            }

            using (ScriptBuilder script = new())
            {
                script.EmitPush(false);
                CollectionAssert.AreEqual(new byte[] { (byte)OpCode.PUSH0, (byte)OpCode.NOT, (byte)OpCode.NOT }, script.ToArray());
            }
        }
