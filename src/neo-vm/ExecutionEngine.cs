...
namespace Neo.VM
{
    public class ExecutionEngine : IDisposable
    {
        ...
        public void StepInto()
        {
            ...
            JumpTable opcode = CurrentContext.InstructionPointer >= CurrentContext.Script.Length ? _jumpTable[0x66] : _jumpTable[CurrentContext.OpReader.ReadByte()];

            try
            {
                opcode.Execute(CurrentContext);
            }
            catch
            {
                State |= VMState.FAULT;
