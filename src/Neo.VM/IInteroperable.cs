...
public interface IInteroperable
{
    ...
    {
        var result = Activator.CreateInstance(GetType()) as IInteroperable
                     ?? throw new InvalidOperationException($"Type {GetType()} must implement {nameof(IInteroperable)} and expose a public parameterless constructor.");
